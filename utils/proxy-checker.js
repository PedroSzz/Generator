...
module.exports = async (proxies, threads, silent = false) => {
	...
	if (!silent) logger.info(`Verificando ${chalk.yellow(proxies.length)} proxies... Isso poderá levar até ${ms((proxies.length * maxRetries * 15000) / threads, { long: true })}.`);
	...
	proxies = await new Promise(complete => {
		...
		const log = () => {
			...
			if (!eta || eta < 60_000) eta = 'Menos de um minuto.';
			...
			process.stdout.write(`${chalk.magenta(time.join(':'))} ${chalk.greenBright('[-]')}  » Proxies restantes: ${proxies.length + threads} | Funcionando: ${checked.length} | Tempo restante: ${eta}      \r`);
			process.title = `Verificando proxies... | Proxies restantes: ${proxies.length + threads} | Funcionando: ${checked.length} | Tempo restante: ${eta}`;
			return;
		};
